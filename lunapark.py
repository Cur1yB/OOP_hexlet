    ...
    def add_visitor(self, visitor: "Visitor"):
        ...
        if visitor._balance >= self.price:
            ...
            if visitor.age >= self.year_limit:
                visitor.withdraw(
                    self.price
                )  # Снимаем стоимость билета с баланса посетителя
                ...
                visitor.tickets.append(
                    ticket
                )  # Добавляем билет к списку билетов посетителя
                print(f"Посетитель {visitor.name} записан на аттракцион {self.name}")
            else:
                print(
                    f"Возраст посетителя {visitor.name} слишком мал для аттракциона {self.name}!"
                )
        else:
            print(
                f"У посетителя {visitor.name} недостаточно денег, чтобы посетить аттракцион {self.name}"
            )
...
american_hills = Attraction(
    year_limit=14, name="Американские горки", price=500, dangerous="R13"
)
...
for visitor in potential_visitors:
    print(
        f"{visitor.name} осталсь денег - {visitor._balance}, потратил денег {functools.reduce(lambda acc, ticket: acc + ticket.attraction.price, visitor.tickets, 0)}"
    )
